...
function kstest() {
	...
	if ( !isNumberArray( x ) && !isTypedArrayLike( x ) ) {
		throw new TypeError( format( '0fiAU', x ) );
	}
	if ( !isFunction( y ) && !isString( y ) ) {
		throw new TypeError( format( '0fiAV', y ) );
	}
	if ( isString( y ) ) {
		y = getCDF( y );
	}
	...
	for ( i = 0; i < nDistParams; i++ ) {
		...
		if ( !isNumber( val ) || isnan( val ) ) {
			throw new TypeError( format( '0fiAW', val ) );
		}
	...
	case 'less':
		...
		break;
	default:
		throw new Error( format( '0fi3t', 'alternative', [ 'two-sided', 'less', 'greater' ].join( '", "' ), alt ) );
	}
	if ( alt === 'two-sided' ) {
