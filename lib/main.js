...
function kstest() {
	...
	if ( !isNumberArray( x ) && !isTypedArrayLike( x ) ) {
		throw new TypeError( format( '1Ld9s,Gd', x ) );
	}
	if ( !isFunction( y ) && !isString( y ) ) {
		throw new TypeError( format( '1Ld9t,Ge', y ) );
	}
	if ( isString( y ) ) {
		y = getCDF( y );
	}
	...
	for ( i = 0; i < nDistParams; i++ ) {
		...
		if ( !isNumber( val ) || isnan( val ) ) {
			throw new TypeError( format( '1Ld9u,Gf', val ) );
		}
	...
	if ( alpha < 0.0 || alpha > 1.0 ) {
		throw new RangeError( format( '1Ld8V,Gb', 'alpha', alpha ) );
	}
	...
	case 'less':
		...
		break;
	default:
		throw new Error( format( '1Ld2X,3g,4S,GD,Gg,Jm', 'alternative', [ 'two-sided', 'less', 'greater' ].join( '", "' ), alt ) );
	}
	if ( alt === 'two-sided' ) {
